...
const sourceProductCount = (html.match(/\{id:'(?:barba|perfume|creme|oleo|roupa|game|ferramenta|tech|acessorio)-\d{2}'/g) || []).length;
...
const failures = [];
if (sourceProductCount !== 109) failures.push(`esperados 109 produtos editoriais na fonte, encontrados ${sourceProductCount}`);
if (links.length !== 20) failures.push(`esperados 20 links, encontrados ${links.length}`);
...
if (imageCount !== 18) failures.push(`esperadas 18 imagens reais, encontradas ${imageCount}`);
...
console.log(JSON.stringify({ ok: true, sourceProductCount, renderedLinkedProducts: 20, affiliateLinks: links.length, realImages: imageCount }, null, 2));
