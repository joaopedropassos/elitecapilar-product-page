...
const sourceProductCount = (html.match(/\{id:'(?:barba|perfume|creme|oleo)-\d{2}'/g) || []).length;
...
const failures = [];
if (sourceProductCount !== 100) failures.push(`esperados 100 produtos editoriais na fonte, encontrados ${sourceProductCount}`);
...
console.log(JSON.stringify({ ok: true, sourceProductCount, renderedLinkedProducts: links.length, affiliateLinks: links.length, realImages: imageCount }, null, 2));
