...
const sourceProductCount = (html.match(/\{id:'(?:barba|perfume|creme|oleo|roupa|game|ferramenta|tech|acessorio|casa|suplemento)-\d{2}'/g) || []).length;
...
const failures = [];
if (sourceProductCount !== 112) failures.push(`esperados 112 produtos editoriais na fonte, encontrados ${sourceProductCount}`);
if (links.length !== 23) failures.push(`esperados 23 links, encontrados ${links.length}`);
...
if (imageCount !== 21) failures.push(`esperadas 21 imagens reais, encontradas ${imageCount}`);
...
console.log(JSON.stringify({ ok: true, sourceProductCount, renderedLinkedProducts: 23, affiliateLinks: links.length, realImages: imageCount }, null, 2));
